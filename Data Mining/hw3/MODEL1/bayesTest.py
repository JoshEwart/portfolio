...
def calcLikely(dataPoint, avg, curCov):
    ...
    firstDenominator = float(1/((2*numPy.pi)**(len(dataPoint)/2)));
    firstDenominator = firstDenominator/(numPy.sqrt(numPy.linalg.det(curCov)));
    #second half
    curAvgDiff = dataPoint-avg;
    inverse = numPy.linalg.inv(curCov);
    transDiff = numPy.transpose(curAvgDiff);
    power = (numPy.dot(curAvgDiff,inverse));
    power = -numPy.dot(power,transDiff);
    secondHalf = numPy.exp(power/2);
    return firstDenominator*secondHalf;
